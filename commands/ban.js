...
exports.run = (client, message, args, language) => {
  if(!message.member.hasPermission('BAN_MEMBERS')){
      const YDHP = new Discord.RichEmbed()
          ...
          .setDescription(lg[language].not_allowed + "BAN_MEMBERS" + lg[language].not_allowed_2)
    ...
    if(serverData[message.guild.id].admin){
      if (!user) {
         const YDHP = new Discord.RichEmbed()
                  .setColor('#ff0000')
                  .setAuthor(client.user.username, client.user.displayAvatarURL)
                  .setDescription(lg[language].unabled_ban)
                  .setFooter(lg[language].deleted_soon)
        ...
        if (member) {
          ...
          reason: banReason,
                  ...
                  .setColor(DC)
                  .setAuthor(client.user.username, client.user.displayAvatarURL)
                  .setDescription(lg[language].banned_successfully + `${banReason}` + '``')
